...
public class ParticleBuilder {
    ...
    /**
     * Creates a new instance of a {@link ParticleBuilder} to be able to easily create Particles
     * @param particle Particle to create
     * @return The ParticleBuilder
     */
    public static ParticleBuilder of(@NotNull Particle particle) {
    ...
    public ParticleBuilder receivers(int radius, boolean byDistance) {
        if (!byDistance) {
            ...
            for (Player nearbyPlayer : Util.getEntitiesAround(location, radius, Player.class)) {
    ...
    public ParticleBuilder receivers(int xzRadius, int yRadius, boolean byDistance) {
        if (!byDistance) {
            ...
            this.receivers = Lists.newArrayList();
            for (Entity nearbyEnt : location.getWorld().getNearbyEntities(location, xzRadius, yRadius, xzRadius, e -> e instanceof Player)) {
    ...
    public ParticleBuilder receivers(int xRadius, int yRadius, int zRadius) {
        ...
        for (Entity entity : location.getWorld().getNearbyEntities(location, xRadius, yRadius, zRadius, e -> e instanceof Player))
            nearbyPlayers.add((Player)entity);
        return receivers(nearbyPlayers);
    }
